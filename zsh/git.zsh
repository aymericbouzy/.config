...
function git-amend-or-commit {
	message="$1"
	base="$(git rev-parse --abbrev-ref --symbolic-full-name @{u} 2>/dev/null || git base)"

	if [ -n "$base" ] && [[ "$(git log "$base..")" = "" ]]; then
		git commit -m "$(echo -e "$message")" --allow-empty
	else
